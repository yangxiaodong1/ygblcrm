...
def render_filter_ele(filter_field,admin_class,filter_conditions):
    ...
    if type(field_obj).__name__ in ["DateTimeField","DateField"]:
        ...
        date_eles.append(["近180天",today_ele-timedelta(days=180)])
        ...
        for item in date_eles:
            select_ele += '''<option value='%s' %s>%s</option>''' %(item[1],selected,item[0])# 都是选中的吗？

        filter_field_name = "%s__gte" % filter_field  # gte是干嘛的？ 大于等于这个时间
    else:
...
def get_m2m_obj_list(admin_class,field,form_obj):
    ...
    all_obj_list = field_obj.rel.to.objects.all() # 不懂rel.to.objects.all()是什么意思

    #单条数据的对象中的某个字段
    if form_obj.instance.id: # 对于增加的时候有这个 判断增加时候是没有这样对象的，这个是表单对象
